...
namespace :db do
  ...
  task :export_data, [:filename] => :environment do |_t, args|
    ...
    filename = args[:filename].presence || Rails.root.join("tmp", "concorsi_dump_#{Time.now.strftime('%Y%m%d_%H%M%S')}.json.gz").to_s
    ...
    # Aggiunge i modelli ActiveStorage se presenti
    if defined?(ActiveStorage::Blob) && ActiveRecord::Base.connection.table_exists?(ActiveStorage::Blob.table_name)
      models << ActiveStorage::Blob
      models << ActiveStorage::Attachment
      models << ActiveStorage::VariantRecord if defined?(ActiveStorage::VariantRecord) && ActiveRecord::Base.connection.table_exists?(ActiveStorage::VariantRecord.table_name)
    end
